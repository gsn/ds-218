...
function startServer(chainId) {
  ...
  app.use('/asset', express.static(servicePath + path.sep + 'dist'));
  ...
  app.get('*', function (request, response) {
    ...
    fs.readFile(indexFile, 'utf8', function (err, str) {
      ...
      str = str.replace('@RenderSection("htmlhead", false)', '<link href="/asset/styles/app.css" rel="stylesheet" />');
      str = str.replace('@RenderBody()', '<script>\n' +
  '(function (globalConfig) {  try {\n' +
  'globalConfig.data = { "ContentBaseUrl": "/asset", "Version": null, "ChainId": ' + chainId + ' };\n' +
  '} catch (e) { }\n' +
  '})(window.globalConfig || {});\n' +
  '</script>\n' +                                                                                                           
  '<script src="/asset/storeApp.js"></script>\n' +
